...
boolean isRegistrationSuccess = WebUI.verifyElementPresent(findTestObject('Register/registrationSuccess'), 0, FailureHandling.OPTIONAL)

try {
	if (isRegistrationSuccess) {
		println('Test Passed')
	} else {
		assert false : 'Test failed: Registration success element not found'
	}
} catch (Exception e) {
